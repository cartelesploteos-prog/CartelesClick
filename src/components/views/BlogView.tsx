...
export const BlogView: React.FC<BlogViewProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="section-container pt-28 sm:pt-32 lg:pt-36 pb-36 sm:pb-44 space-y-12 sm:space-y-16 max-w-6xl font-sans">
      
      {/* ARTICLE READER MODAL */}
      {activeArticle && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-[7px] max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 space-y-6">
            <div className="flex items-center justify-between border-b border-[var(--border-subtle)] pb-4">
              ...
              <span className="text-xs px-2.5 py-1 rounded-[7px] bg-primary/10 text-primary font-medium">
                {activeArticle.tag}
              </span>
            </div>

            <div className="space-y-4">
              <div className="h-64 rounded-[7px] overflow-hidden">
                <img
              ...
              <h1 className="font-heading text-2xl sm:text-3xl text-[var(--text-primary)] font-medium leading-tight">
                {activeArticle.title}
              </h1>
            ...
            <div className="pt-6 border-t border-[var(--border-subtle)] flex items-center justify-between">
              <button
                ...
                className="px-5 py-2.5 rounded-[7px] bg-primary text-white text-xs font-medium hover:bg-primary/90 transition-colors"
              >
              ...
              <button
                onClick={() => setActiveArticle(null)}
                className="px-4 py-2 rounded-[7px] border border-[var(--border-subtle)] text-xs text-[var(--text-secondary)]"
              >
                Cerrar
              </button>
            </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[var(--border-subtle)] pb-6">
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-[7px] border border-[var(--border-subtle)] bg-[var(--bg-surface)] hover:bg-[var(--bg-surface-subtle)] text-xs font-medium text-[var(--text-primary)] flex items-center gap-1.5 transition-colors self-start md:self-auto shadow-none"
        >
