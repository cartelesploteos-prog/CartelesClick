...
const regex = /app\.post\('\/api\/quote-batch', quoteRateLimiter, \(req, res\) => \{[\s\S]*?return res\.status\(500\)\.json\(\{ error: 'Error interno al procesar cotización de lote\.' \}\);\n  \}\n\}\);/g;
...
if (match) {
  ...
  try {
    ...
    // Map back to original items array order
